...
public class EduSubjectServiceImpl extends ServiceImpl<EduSubjectMapper, EduSubject> implements EduSubjectService {
    ...
    public void saveSubject(MultipartFile file,EduSubjectService subjectService) {
        try {
            ...
            //调用方法进行读取
            EasyExcel.read(in, SubjectData.class,new SubjectExcelListener1(subjectService)).sheet().doRead();
    ...
    public List<OneSubject> getAllOneTwoSubject() {
        ...
        //3.封装一级分类
        //查询出来所有的一级发呢类list集合遍历，得到每个一级分类对象，获取每个一级分类对象值，
        ...
        for(int i=0;i<oneSubjectList.size();i++){
            ...
            //吧得到eduSubject里面值获取出来，放到OneSubject对象里面
...
//            oneSubject.setTitle(eduSubject.getTitle());
            ...
            for (int m=0;m<twoSubjectList.size();m++){
                ...
                {
                    //吧tSubject值复制到TwoSubject里面，放到twoFinalSubjectList里面
    ...
    public void saveTest(MultipartFile file,EduSubjectService subjectService) {
        try {
            ...
            InputStream in = file.getInputStream();
            //调用方法进行读取
            EasyExcel.read(in, TestData.class,new SubjectExcelListener1(subjectService)).sheet().doRead();
